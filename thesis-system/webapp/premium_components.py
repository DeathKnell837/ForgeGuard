...
Sophos AI Smart Cybersecurity Visual Identity (v6.0)
"""

def render_sophos_brand_sidebar():
    return """<div style="padding: 0.2rem 0.2rem 0.6rem 0.2rem; margin-bottom: 0.2rem;">
<div style="display: flex; align-items: center; gap: 10px;">
<div style="width: 34px; height: 34px; border-radius: 9px; background: linear-gradient(135deg, #7C6FF0 0%, #4F46E5 100%); display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 14px rgba(124, 111, 240, 0.4);">
...
def render_investigator_profile_card():
    return """<div style="border-top: 1px solid rgba(255, 255, 255, 0.06); margin-top: 1.8rem; padding-top: 1rem; padding-left: 0.2rem; padding-right: 0.2rem;">
...
<span style="color: #9CA3AF;">Global Acc:</span> <strong style="color: #7C6FF0;">{accuracy_pct}%</strong>
</div>
...
def render_exhibit_metadata_bar(filename, resolution, sha256_hash):
    return f"""<div style="display: flex; justify-content: space-between; align-items: center; background: #161922; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 10px; padding: 7px 16px; margin-bottom: 9px; font-family: 'Inter', sans-serif; font-size: 0.74rem; color: #9CA3AF; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.15);">
...
def render_panoramic_incident_cockpit(verdict_text, is_forged, confidence, ela_mean, ela_var, ela_max, gemini_analysis=None):
    ...
    status_border = "rgba(239, 68, 68, 0.3)" if is_forged else "rgba(16, 185, 129, 0.3)"
    ...
    analysis_block = ""
    if gemini_analysis:
        analysis_block = f"""<div style="background: #1A1D26; border: 1px solid rgba(255, 255, 255, 0.08); border-left: 3px solid #7C6FF0; border-radius: 10px; padding: 12px 16px; margin-top: 14px;">
...
<div style="display: grid; grid-template-columns: 1.15fr 1fr; gap: 20px; align-items: center;">
<div>
<div style="font-family: 'Inter', sans-serif; font-size: 1.3rem; font-weight: 800; color: #FFFFFF; letter-spacing: 0.2px; margin-bottom: 4px;">{verdict_text}</div>
<div style="font-size: 0.78rem; color: #9CA3AF; line-height: 1.45; margin-bottom: 14px;">{sub_desc}</div>

<div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; text-align: center;">
<div style="background: #1A1D26; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 10px; padding: 8px 6px;">
...
<div style="font-family: 'Inter', sans-serif; font-size: 1.05rem; font-weight: 700; color: #7C6FF0;">{ela_mean:.1f}</div>
...
<div style="background: #1A1D26; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 12px; padding: 12px 16px;">
<div style="display: flex; justify-content: space-between; font-size: 0.7rem; font-family: 'Inter', sans-serif; font-weight: 600; color: #9CA3AF; margin-bottom: 10px; text-transform: uppercase;">
...
<div>
<div style="display: flex; justify-content: space-between; font-family: 'Inter', sans-serif; font-size: 0.78rem; margin-bottom: 4px;">
...
<div class="bench-kpi-sub" style="color: #2DD4BF;">MobileNetV2 (Edge Optimized)</div>
</div>
</div>
...
<div class="bench-kpi-sub" style="color: #10B981;">MobileNetV2 (3.4M vs 23.5M)</div>
</div>
</div>

<div class="bench-kpi-card" style="border-color: rgba(124, 111, 240, 0.4); background: #161922;">
<div style="display: flex; justify-content: space-between; align-items: flex-start;">
<div class="bench-kpi-icon-chip" style="background: rgba(124, 111, 240, 0.15); border-color: rgba(124, 111, 240, 0.3);">
<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#7C6FF0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
<div>
<div class="bench-kpi-label" style="color: #7C6FF0;">SOP 5 Selected Model</div>
<div class="bench-kpi-value" style="color: #7C6FF0; font-size: 1.55rem;">MobileNetV2</div>
<div class="bench-kpi-sub" style="color: #FFFFFF;">Optimal Smartphone Deployment</div>
</div>
</div>
</div>"""
...
def svg_radial_dial(percent, color="#7C6FF0", label="ACCURACY", size=110):
    ...
    return f"""<div style="display: flex; justify-content: center; margin: 0.8rem 0;">
...
<text x="60" y="75" text-anchor="middle" fill="{color}" font-family="'Inter', sans-serif" font-size="8" font-weight="700" letter-spacing="1.2">{label}</text>
</svg>
</div>"""


def render_saas_model_card(title, tag, acc, prec, rec, f1, speed, params, comp_acc, color="#7C6FF0", is_recommended=False):
    rec_badge = f"""<span style="background:rgba(124, 111, 240, 0.15);color:{color};font-family:'Inter',sans-serif;font-size:0.68rem;font-weight:700;padding:4px 9px;border-radius:6px;border:1px solid rgba(124, 111, 240, 0.3);">{tag}</span>"""
...
<span style="font-family: 'Inter', sans-serif; font-weight: 700; font-size: 1rem; color: #FFFFFF;">{title}</span>
{rec_badge}
</div>
{dial_html}
...
<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; background: #1A1D26; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 10px; padding: 8px; text-align: center;">
<div>
...
<div style="font-size: 0.9rem; font-weight: 700; color: #FFFFFF; font-family: 'Inter', sans-serif;">{speed}</div>
</div>
<div>
...
<div style="font-size: 0.9rem; font-weight: 700; color: #FFFFFF; font-family: 'Inter', sans-serif;">{params}</div>
</div>
</div>
</div>"""
...
def render_comparative_breakdown_bars():
    return """<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-top: 1.1rem;">
<div style="background: #161922; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 14px; padding: 1.2rem 1.4rem; box-shadow: 0 4px 16px rgba(0, 0, 0, 0.2);">
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
<span style="font-family: 'Inter', sans-serif; font-size: 0.9rem; font-weight: 700; color: #FFFFFF;">Inference Speed on Mobile CPU</span>
<span style="font-family: 'Inter', sans-serif; font-size: 0.7rem; color: #7C6FF0; font-weight: 600;">Milliseconds</span>
</div>
<div style="display: flex; flex-direction: column; gap: 10px;">
<div style="display: flex; align-items: center; gap: 10px;">
...
<span style="width: 65px; text-align: right; font-family: 'Inter', sans-serif; font-size: 0.8rem; font-weight: 700; color: #7C6FF0;">12.4 ms</span>
</div>
<div style="display: flex; align-items: center; gap: 10px;">
...
<span style="width: 65px; text-align: right; font-family: 'Inter', sans-serif; font-size: 0.8rem; font-weight: 700; color: #2DD4BF;">28.6 ms</span>
</div>
<div style="display: flex; align-items: center; gap: 10px;">
...
<span style="width: 65px; text-align: right; font-family: 'Inter', sans-serif; font-size: 0.8rem; font-weight: 700; color: #F59E0B;">45.2 ms</span>
</div>
</div>
</div>
...
<span style="font-family: 'Inter', sans-serif; font-size: 0.7rem; color: #10B981; font-weight: 600;">Millions of Params</span>
</div>
<div style="display: flex; flex-direction: column; gap: 10px;">
<div style="display: flex; align-items: center; gap: 10px;">
<span style="width: 105px; font-size: 0.78rem; color: #FFFFFF; font-weight: 600;">MobileNetV2</span>
...
<span style="width: 65px; text-align: right; font-family: 'Inter', sans-serif; font-size: 0.8rem; font-weight: 700; color: #10B981;">3.4 M</span>
</div>
<div style="display: flex; align-items: center; gap: 10px;">
<span style="width: 105px; font-size: 0.78rem; color: #9CA3AF;">Basic CNN</span>
<div class="progress-track-dark" style="flex: 1; height: 8px;">
...
<span style="width: 65px; text-align: right; font-family: 'Inter', sans-serif; font-size: 0.8rem; font-weight: 700; color: #F59E0B;">2.1 M</span>
</div>
<div style="display: flex; align-items: center; gap: 10px;">
<span style="width: 105px; font-size: 0.78rem; color: #9CA3AF;">ResNet50</span>
<div class="progress-track-dark" style="flex: 1; height: 8px;">
...
<span style="width: 65px; text-align: right; font-family: 'Inter', sans-serif; font-size: 0.8rem; font-weight: 700; color: #2DD4BF;">23.5 M</span>
</div>
</div>
</div>
</div>"""
...
def executive_sop5_recommendation_card():
    return """<div style="background: #161922; border: 1px solid rgba(255, 255, 255, 0.08); border-left: 4px solid #7C6FF0; border-radius: 14px; padding: 1.2rem 1.5rem; margin-top: 1.1rem; box-shadow: 0 4px 16px rgba(0, 0, 0, 0.2);">
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
<div style="font-family: 'Inter', sans-serif; font-size: 1rem; font-weight: 800; color: #7C6FF0; letter-spacing: 0.2px;">Thesis SOP 5 — Optimal Architecture Conclusion</div>
<span style="background: rgba(124, 111, 240, 0.15); color: #7C6FF0; font-family: 'Inter', sans-serif; font-size: 0.7rem; font-weight: 700; padding: 4px 11px; border-radius: 6px; border: 1px solid rgba(124, 111, 240, 0.3);">MobileNetV2 Selected</span>
</div>
<div style="color: #E2E8F0; font-size: 0.86rem; line-height: 1.55; margin-bottom: 0.6rem;">
...
<div style="font-family: 'Inter', sans-serif; font-size: 0.8rem; color: #10B981; background: rgba(16, 185, 129, 0.08); padding: 8px 12px; border-radius: 8px; border: 1px solid rgba(16, 185, 129, 0.25);">
