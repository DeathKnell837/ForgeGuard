...
No JavaScript required — all animations are CSS @keyframes.
"""


def svg_confidence_gauge(confidence_pct, verdict_color, verdict_label):
    """
    Renders a premium SVG circular confidence gauge (speedometer style).
    
    Args:
        confidence_pct: Float 0-100 (e.g. 87.3)
        verdict_color: Hex color string (e.g. '#34D399' for authentic, '#F87171' for forged)
        verdict_label: 'AUTHENTIC' or 'FORGED'
    
    Returns:
        HTML string with embedded SVG gauge
    """
    # SVG arc calculation: circumference of circle with r=80 is 2*pi*80 ≈ 502.65
    # We use 270 degrees of the circle (3/4), so arc length = 502.65 * 0.75 ≈ 376.99
    arc_length = 376.99
    filled = arc_length * (confidence_pct / 100.0)
    gap = arc_length - filled
    
    # Glow filter intensity based on confidence
    ...
    <div class="gauge-container">
        <svg viewBox="0 0 200 200" class="gauge-svg">
            ...
            <circle cx="100" cy="100" r="80" fill="none" 
                ...
                stroke-dasharray="{arc_length} {502.65 - arc_length}"
                stroke-dashoffset="-{502.65 * 0.125}"
            ...
            <circle cx="100" cy="100" r="80" fill="none" 
                ...
                stroke-dasharray="{filled} {gap + (502.65 - arc_length)}"
                stroke-dashoffset="-{502.65 * 0.125}"
                stroke-linecap="round"
                filter="url(#gaugeGlow)"
                class="gauge-arc"/>
            
            <!-- Tick marks (small lines at intervals) -->
            ...
            <!-- Center text -->
            <text x="100" y="92" text-anchor="middle" fill="{verdict_color}" 
                font-family="'JetBrains Mono', 'IBM Plex Mono', monospace" 
                font-size="28" font-weight="700">{confidence_pct:.1f}%</text>
            <text x="100" y="115" text-anchor="middle" fill="{verdict_color}" 
                font-family="'JetBrains Mono', 'IBM Plex Mono', monospace" 
                font-size="9" font-weight="600" letter-spacing="2.5"
                opacity="0.85">{verdict_label}</text>
        </svg>
    </div>
    """
...
def premium_verdict_stamp(verdict_text, stamp_class, sub_reason, verdict_color, verdict_label, confidence, model_name, latency_ms):
    """
    Renders the embossed rubber stamp verdict with slam animation.
    """
    return f"""
    <div class="stamp-container">
        ...
        <div class="stamp-meta-bar">
            ...
            <span>[CONFIDENCE: <strong style="color: {verdict_color};">{confidence * 100:.1f}%</strong>]</span>
            <span>[MODEL: <strong style="color: #F8FAFC;">{model_name.upper()}</strong>]</span>
            <span>[LATENCY: <strong style="color: #2DD4BF;">{latency_ms:.1f}ms</strong>]</span>
        </div>
    </div>
    """
...
def premium_metric_card(value, label, color="#2DD4BF", icon_type="bar"):
    """
    Renders a frosted glass metric card with mini sparkline SVG.
    """
    # Mini sparkline bars SVG
    if icon_type == "bar":
...
def premium_arch_card(name, score, latency, params, is_active=False, is_forged=False):
    """
    Renders an architecture comparison card with animated progress bar.
    """
    score_pct = score * 100
    bar_color = "#F87171" if is_forged else "#34D399"
    active_border = f"border-left: 3px solid #2DD4BF;" if is_active else ""
    active_badge = '<span style="background: rgba(45,212,191,0.15); color: #2DD4BF; font-size: 0.65rem; padding: 2px 8px; border-radius: 4px; font-weight: 700; letter-spacing: 0.5px;">ACTIVE</span>' if is_active else ""
    verdict_badge = "FORGED" if is_forged else "AUTHENTIC"
    badge_color = "#F87171" if is_forged else "#34D399"
    
    return f"""
    <div class="glass-panel-matrix" style="{active_border}">
        <div>
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
                <div class="serif-header" style="font-size: 1.05rem; color: #F8FAFC;">{name}</div>
                <span style="color: {badge_color}; font-family: 'JetBrains Mono', monospace; font-size: 0.68rem; font-weight: 700;">[{verdict_badge}]</span>
            </div>
            <div style="color: #64748B; font-size: 0.75rem; margin-bottom: 0.75rem;">{active_badge}</div>
            
            <!-- Confidence Score -->
            <div style="font-family: 'JetBrains Mono', monospace; font-size: 1.6rem; font-weight: 700; color: {badge_color}; margin-bottom: 0.5rem;">
                {score_pct:.1f}%
            </div>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.72rem; color: #64748B;">
                <span style="color: #94A3B8;">Latency:</span> <strong style="color: #2DD4BF;">{latency}ms</strong>
            </div>
            <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.72rem; color: #64748B;">
                <span style="color: #94A3B8;">Params:</span> <strong style="color: #F8FAFC;">{params}</strong>
            </div>
        </div>
    </div>
    """
...
def premium_header_bar(svg_shield):
    """
    Renders the premium frosted-glass header brand bar with animated shield and gold shimmer line.
    """
    return f"""
...
def premium_hero_banner():
    """
    Renders the premium hero dashboard banner.
    """
    return """
    <div class="glass-panel hero-panel">
        <div class="eyebrow-gold" style="display: flex; align-items: center; gap: 8px;">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#C9A15F" stroke-width="2.5" stroke-linecap="round">
        ...
        <div class="serif-header" style="font-size: 1.65rem; color: #F8FAFC; margin-bottom: 0.5rem; line-height: 1.2;">
            Digital Receipt Forgery Detection<br>& Forensic Suite
        </div>
        <div style="color: #94A3B8; font-size: 0.86rem; line-height: 1.6; margin-bottom: 1rem; max-width: 820px;">
...
def inference_mode_badge(inference_mode, mode_color):
    """
    Renders the inference engine mode diagnostic badge.
    """
    mode_desc = {
        "CNN": "Trained .keras neural network active",
        "FALLBACK": "ELA regional differential heuristic",
    }.get(inference_mode, "Processing...")
    ...
    return f"""
    <div style="display: flex; align-items: center; justify-content: center; gap: 8px; padding: 6px 16px; background: rgba({int(mode_color[1:3],16)},{int(mode_color[3:5],16)},{int(mode_color[5:7],16)},0.08); border: 1px solid rgba({int(mode_color[1:3],16)},{int(mode_color[3:5],16)},{int(mode_color[5:7],16)},0.25); border-radius: 8px; margin: 0.5rem auto 1rem auto; width: fit-content;">
        <span style="color: {mode_color};">{icon}</span>
        <span style="font-family: 'JetBrains Mono', monospace; font-size: 0.72rem; color: {mode_color}; letter-spacing: 0.5px;">
            INFERENCE ENGINE: <strong>{inference_mode}</strong> — {mode_desc}
        </span>
    </div>
    """
